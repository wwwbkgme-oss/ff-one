...
impl EconomyEngine for EconomyStore {
    ...
    async fn purchase(&self, lid: Uuid, buyer_id: Uuid, qty: u64) -> Result<()> {
        ...
        let cost  = s.listings.get(&lid)
            .ok_or_else(|| FfError::ListingNotFound(lid.to_string()))?
            .price_per_unit.saturating_mul(qty);
        let seller_id = s.listings.get(&lid).unwrap().seller_id;
        let bw = s.wallets.entry(buyer_id).or_default();
        if !bw.debit(cost) {
            return Err(FfError::InsufficientFunds { needed: cost, available: bw.balance });
        }
        s.wallets.entry(seller_id).or_default().credit(cost);
        if let Some(l) = s.listings.get_mut(&lid) { l.active = false; }
        Ok(())
    }

    async fn get_listings(&self) -> Result<Vec<MarketListing>> {
        Ok(self.inner.read().await.listings.values().filter(|l| l.active).cloned().collect())
    }

    async fn create_auction(&self, seller_id: Uuid, resource: Resource, start_price: u64, dur: u64) -> Result<AuctionItem> {
        let a = AuctionItem::new(seller_id, resource, start_price, Utc::now() + Duration::seconds(dur as i64));
    ...
    async fn place_bid(&self, aid: Uuid, bidder_id: Uuid, amount: u64) -> Result<()> {
        let mut s = self.inner.write().await;
        let a = s.auctions.get_mut(&aid).ok_or_else(|| FfError::ListingNotFound(aid.to_string()))?;
        if a.closed { return Err(FfError::AuctionClosed(aid.to_string())); }
        if !a.place_bid(bidder_id, amount) {
            return Err(FfError::InsufficientFunds { needed: a.current_bid + 1, available: amount });
        }
        Ok(())
    }

    async fn get_auctions(&self) -> Result<Vec<AuctionItem>> {
        Ok(self.inner.read().await.auctions.values().filter(|a| !a.closed).cloned().collect())
    }

    async fn get_wallet(&self, id: Uuid) -> Result<Wallet> {
        Ok(self.inner.read().await.wallets.get(&id).cloned().unwrap_or_default())
    }
    ...
    async fn transfer(&self, from: Uuid, to: Uuid, amount: u64) -> Result<()> {
        ...
        if !fw.debit(amount) {
            return Err(FfError::InsufficientFunds { needed: amount, available: fw.balance });
        }
...
mod tests {
    ...
    async fn wallet_transfer_ok() {
        ...
        let a = Uuid::new_v4();
        let b = Uuid::new_v4();
        store.inner.write().await.wallets.insert(a, Wallet { balance: 100 });
