...
impl QuestManager for QuestStore {
    ...
    async fn accept_quest(&self, qid: Uuid, agent_id: Uuid) -> Result<()> {
        ...
        let q = qs
            .get_mut(&qid)
            .ok_or_else(|| FfError::QuestNotFound(qid.to_string()))?;
        if !q.is_available() {
            return Err(FfError::QuestUnavailable(qid.to_string()));
        }
    ...
    async fn generate_quest(&self, world: &WorldState, _agent: &Agent) -> Result<Quest> {
        let (title, desc, obj) = match world.epoch.number % 4 {
            ...
            1 => (
                "Gold verdienen",
                "Verdiene 50 Gold durch Handel.",
                QuestObjective::Earn { amount: 50 },
            ),
            2 => (
                "Expedition",
                "Erkunde die Vulkanöde.",
                QuestObjective::Explore {
                    target: "VolcanicWastes".into(),
                    radius: 50,
                },
            ),
            _ => (
                "Code-Herausforderung",
                "Berechne Fibonacci(10).",
                QuestObjective::Freeform {
                    description: "Fibonacci(10)".into(),
                },
            ),
        };
        let q = Quest::new(
