...
impl StaticAnalyser {
    pub fn new() -> Self {
        let rules = vec![
            Rule {
                pattern: Regex::new(r"(?i)(os\.system|subprocess\.|exec\(|eval\()").unwrap(),
                category: FindingCategory::DangerousSyscall,
                severity: Severity::High,
                title: "OS-Befehlsausführung",
                desc: "Möglicher Shell-Aufruf.",
            },
            Rule {
                pattern: Regex::new(r"(?i)(socket\.|urllib|requests\.|http\.client|fetch\()")
                    .unwrap(),
                category: FindingCategory::NetworkAccess,
                severity: Severity::Medium,
                title: "Netzwerkzugriff",
                desc: "Verbindungsversuch erkannt.",
            },
            Rule {
                pattern: Regex::new(r"(?i)(open\s*\(|pathlib|shutil\.|os\.path)").unwrap(),
                category: FindingCategory::FilesystemAccess,
                severity: Severity::Low,
                title: "Dateisystemzugriff",
                desc: "Lese- oder Schreiboperation.",
            },
            Rule {
                pattern: Regex::new(r"(?i)(import\s+ctypes|cffi|mmap\.)").unwrap(),
                category: FindingCategory::DangerousSyscall,
                severity: Severity::Critical,
                title: "Nativer Speicherzugriff",
                desc: "ctypes/cffi verwendet.",
            },
            Rule {
                pattern: Regex::new(r"while\s+True|while\s+1").unwrap(),
                category: FindingCategory::ResourceAbuse,
                severity: Severity::Medium,
                title: "Mögliche Endlosschleife",
                desc: "CPU-Erschöpfung möglich.",
            },
            Rule {
                pattern: Regex::new(r"(?i)rm\s+-rf|shutil\.rmtree").unwrap(),
                category: FindingCategory::FilesystemAccess,
                severity: Severity::High,
                title: "Rekursives Löschen",
                desc: "Dateien werden rekursiv gelöscht.",
            },
        ];
        Self {
            rules,
    ...
    fn decide_inner(&self, findings: &[SecurityFinding]) -> SafetyDecision {
        ...
        if score >= self.threshold {
            SafetyDecision::Reject {
                reason: format!("Risiko-Score {score}"),
            }
        } else if score > 0 {
            SafetyDecision::ExecuteRestricted {
                reason: format!("Risiko-Score {score}"),
            }
        } else {
            SafetyDecision::Safe
        ...
        for (line_no, line) in code.lines().enumerate() {
            for r in &self.rules {
                if r.pattern.is_match(line) {
                    let mut f =
                        SecurityFinding::new(r.category.clone(), r.severity, r.title, r.desc);
