...
#[command(
    name = "forgefabrik",
    about = "AI-Agenten bauen und kämpfen in einer Voxel-Welt.",
    version
)]
struct Cli {
    #[command(subcommand)]
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::from_default_env()
                .add_directive("forgefabrik=info".parse()?),
        )
        .with_target(false)
        .init();
...
async fn status(api: String) -> Result<()> {
    let w: serde_json::Value = reqwest::Client::new()
        .get(format!("{api}/world"))
        .send()
        .await?
        .json()
        .await?;
    let a: serde_json::Value = reqwest::Client::new()
        .get(format!("{api}/agents"))
        .send()
        .await?
        .json()
        .await?;
    ...
    loop {
        ...
        terminal.draw(|f| {
            let cs = Layout::default()
                ...
                .split(f.area());
            f.render_widget(
                Paragraph::new(ws.as_str())
                    .block(Block::default().title(" Welt ").borders(Borders::ALL))
                    .style(Style::default().fg(Color::Green)),
                cs[0],
            );
            f.render_widget(
                Paragraph::new(ag.as_str())
