...
impl ClaudeDriver {
    pub fn new(api_key: impl Into<String>) -> Self {
        Self { client: reqwest::Client::new(), api_key: api_key.into(), model: "claude-opus-4-5".into() }
    }
...
impl AgentDriver for ClaudeDriver {
    ...
    fn capabilities(&self) -> Vec<AgentCapability> {
        vec![AgentCapability::BuildAndMine, AgentCapability::ExecuteCode,
             AgentCapability::Trade, AgentCapability::Witness, AgentCapability::QuestGiver]
    }

    async fn complete(&self, agent: &Agent, prompt: &str) -> Result<String> {
        let sys = agents::prompt::system_prompt(agent);
        let body = Req { model: &self.model, max_tokens: 1024, system: &sys,
        ...
        Ok(data.content.into_iter().map(|c| c.text).collect::<Vec<_>>().join(""))
    }
    ...
    async fn is_available(&self) -> bool {
        self.client.get("https://api.anthropic.com").send().await
            .map(|r| r.status().as_u16() < 500).unwrap_or(false)
    }
}
