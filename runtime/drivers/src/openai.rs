...
use contracts::{error::{FfError, Result}, traits::AgentDriver};
use serde::{Deserialize, Serialize};
use types::agent::{Agent, AgentCapability};

#[derive(Serialize)]
struct Req<'a> { model: &'a str, messages: Vec<Msg<'a>>, max_tokens: u32 }
#[derive(Serialize)]
...
impl OpenAiDriver {
    pub fn new(name: impl Into<String>, url: impl Into<String>, key: impl Into<String>, model: impl Into<String>) -> Self {
        Self { client: reqwest::Client::new(), driver_name: name.into(), base_url: url.into(), api_key: key.into(), model: model.into() }
    }
    ...
    pub fn codex(key: impl Into<String>) -> Self {
        Self::new("Codex", "https://api.openai.com/v1", key, "code-davinci-002")
    }
}
...
impl AgentDriver for OpenAiDriver {
    ...
    fn capabilities(&self) -> Vec<AgentCapability> {
        vec![AgentCapability::BuildAndMine, AgentCapability::ExecuteCode,
             AgentCapability::Trade, AgentCapability::Witness]
    }

    async fn complete(&self, agent: &Agent, prompt: &str) -> Result<String> {
        let sys = agents::prompt::system_prompt(agent);
        let body = Req { model: &self.model, max_tokens: 1024,
                         messages: vec![Msg{role:"system",content:&sys}, Msg{role:"user",content:prompt}] };
        ...
        let data: Resp = resp.json().await.map_err(|e| FfError::Other(e.to_string()))?;
    ...
    async fn generate_code(&self, agent: &Agent, task: &str, lang: &str) -> Result<String> {
        self.complete(agent, &agents::prompt::code_prompt(task, lang)).await
    }
